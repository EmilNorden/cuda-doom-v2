...
void transfer_vector_to_device_memory(const ContainerType<T, AllocatorType> &items, T **device_memory) {
    cuda_assert(cudaMallocManaged(device_memory, sizeof(T) * items.size()));
