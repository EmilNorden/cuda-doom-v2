...
static void bw_expand_capacity(binary_writer_t *writer) {
    writer->data = realloc(writer->data, writer->capacity * 2);
...
static boolean bw_write32(binary_writer_t *writer, uint32_t value) {
    const int WORD_SIZE = 4;
    while (writer->capacity - writer->write_position < WORD_SIZE) {
        bw_expand_capacity(writer);
    }

    uint32_t *write_ptr = (uint32_t *) &writer->data[writer->write_position];
    *write_ptr = value;
    writer->write_position += WORD_SIZE;
}

static boolean bw_write16(binary_writer_t *writer, uint16_t value) {
    ...
    uint16_t *write_ptr = (uint16_t *) &writer->data[writer->write_position];
    *write_ptr = value;
    writer->write_position += WORD_SIZE;
}

static boolean bw_write8(binary_writer_t *writer, uint8_t value) {
...
static boolean write_header(binary_writer_t *writer) {
    const int MAGIC_HEADER = 0x6468544D;
    if (!bw_write32(writer, MAGIC_HEADER)) {
        return false;
    }

    // Length
    if (!bw_write32(writer, Reverse32(6))) {
        return false;
    }

    // Format
    if (!bw_write16(writer, Reverse16(0))) {
        return false;
    }

    // Number of tracks
    if (!bw_write16(writer, Reverse16(1))) {
        return false;
    }

    // Number of ticks per quarter note
    if (!bw_write16(writer, Reverse16(70))) {
        return false;
    }
...
boolean convert_mus_to_midi(struct mus_file *mus, midi_data_t *out_midi) {
